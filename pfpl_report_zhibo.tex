...
ensure the type safety during execution. Weakending is crucial for the derivation tree especially
...
\end{lemma}
\begin{proof}
    \textbf{Note: There is one thing that I don't understand about the proof of the lemma. 
    Namely, how we interpret and handle contexts that are extended by a variable. say 
    VAR-REFL, The normal proof would show that $\Gamma, x:\tau, e:\tau' \vdash e:\tau'$. 
    But I think the weakening lemma actually wants us to prove 
    $\Gamma, e:\tau', x:\tau \vdash e:\tau'$. I don't see how we can prove that. I
    would need to meet with the professor to discuss how I can prove it. For the rest of 
    this paper, I would assume that this theorem has been proved. }
...
\begin{proof}
    ...
    \begin{enumerate}[(i)]
        ...
        where $\Gamma, x:\tau' \vdash e' : \bN$,
        by the inductive hypothesis, $\Gamma, x:\tau' \vdash [e_2/x]e' : \bN$, 
        so $\Gamma, x:\tau' \vdash s([e_2/x]e'): \bN$ by NAT-INTRO-S,
        then $\Gamma, x:\tau' \vdash [e_2/x]s(e'): \bN$ by rules of substitution,
        then  $\Gamma, x:\tau' \vdash [e_2/x]e_1: \bN$ since $e_1 = s(e')$. 
        \item NAT-ELIM

        In this case, $e_1 = \rec\{e';x',y.e''\}(e''')$ has type $\tau$, 
        where 

        (a)$\Gamma, x:\tau' \vdash e''' : \bN$,

        (b)$\Gamma, x:\tau' \vdash e' : \tau$,

        and (c)$\Gamma, x:\tau', x':\bN, y:\tau \vdash e''' : \tau$.

        By the inductive hypothesis (and permutation lemma), we get 

        (a)$\Gamma, x:\tau' \vdash [e_2/x]e''' : \bN$,

        (b)$\Gamma, x:\tau' \vdash [e_2/x]e' : \tau$,

        and (c)$\Gamma, x:\tau', x':\bN, y:\tau \vdash [e_2/x]e''' : \tau$, 

        so 
        
        $\Gamma, x:\tau' \vdash \rec\{[e_2/x]e';x',y.[e_2/x]e''\}([e_2/x]e'''): \tau$ by NAT-ELIM of (a)(b)(c),
        then 
        
        $\Gamma, x:\tau' \vdash [e_2/x]\rec\{e';x',y.e''\}(e'''): \tau$ by rules of substitution.

        since  $e_1 = \rec\{e';x',y.e''\}(e''')$ 
        we get  $\Gamma, x:\tau' \vdash [e_2/x]e_1: \tau$ 
        ...
        By inductive hypothesis (and permutation lemma), 
        $\Gamma, x:\tau', x': \tau_1 \vdash [e_2/x]e' : \tau_2$. 

        Then $\Gamma, x:\tau', x':\tau_1 \vdash \lambda (x': \tau_1) [e_2/x]e' : \tau_1 \to \tau_2$,
        by ARROW-INTRO. 

        Then $\Gamma, x:\tau', x':\tau_1 \vdash [e_2/x](\lambda (x':\tau_1) e'): \tau_1 \to \tau_2$ 
        by the rules of substitution. 

        Since $e_1 = \lambda (x':\tau_1) e'$, we get

        $\Gamma, x:\tau', x':\tau_1 \vdash [e_2/x]e_1: \tau_1 \to \tau_2$.  
